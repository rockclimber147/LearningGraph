...
def interpret_notes(notes_text: str):
    """Interprets notes by generating a summary and yields speed data."""
    if not llm:
    ...
    # 1. Yield the standard output tokens
    ...
    # 2. Calculate T/s (handle division by zero if time is near-instant)
    if total_time > 0:
        tokens_per_second = token_count / total_time
    else:
        tokens_per_second = 0 
        
    # 3. Yield the speed data as a separate, structured block (optional but helpful)
    yield f"\n\n--- SPEED METRICS ---\nTotal Tokens Generated: {token_count}\nTotal Time: {total_time:.2f} seconds\nTokens Per Second (T/s): {tokens_per_second:.2f}"
...
if __name__ == '__main__':
    ...
    print("\n--- Interpretation ---")
    for token in interpret_notes(sample_notes):
        print(token, end="")