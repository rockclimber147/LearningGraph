...
export default function MarkdownMetaDataComponent({
  metadata,
  onMetaDataChange,
}: MetaDataFormProps) {
  const handleArrayChange = (field: keyof MarkdownMetaData, value: string) => {
  ...
  const inputClasses =
    "w-full p-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out";
  ...
  return (
    <div className="p-4 rounded-lg shadow-md mb-4 border border-gray-200">
      ...
      <label className={labelClasses}>
        Prerequisites (File Names, Comma Separated)
      </label>
      <input
        type="text"
      ...
      <label className={labelClasses}>
        Related Topics (File Names, Comma Separated)
      </label>
      <input
        type="text"
