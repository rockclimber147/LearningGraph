...
export class TreeBuilder implements GraphBuilder {
  ...
  private getDepthScaledSpacing(
    ...
    const scaledDistance =
      (this.BASE_SPACING_DISTANCE - this.MIN_SPACING_DISTANCE) * decayFactor +
      this.MIN_SPACING_DISTANCE;
  ...
  private traverseAndBuildGraph(
    ...
    if (numChildren > 0) {
      ...
      children.forEach((child, index) => {
        const childAngle =
          startAngle + index * angleStep * (1 - ((2 * index) % 2));
